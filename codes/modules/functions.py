...
def spectral_localizer_AII2D(syst, W, E0, TR,
    ...
    if compute_DOS==True:
        return energy_subset, density_subset_average,localgap_average,list_localgap_realizations
...
def sparse_spectral_localizer_AII3D(ham, fsyst_sites, W, E0,
                                    X0=np.array(['None']), p=params, 
                                    compute_inv=True, compute_DOS=False, 
    ...
    print('DISORDER. W:',W,', sigma:',sigma,', N_dimeroff:',N_dimeroff, ' & num_reals:',num_reals)
    ...
    D = sp.kron(X-x0*id,sigma_x) + sp.kron(Y-y0*id,sigma_y) + sp.kron(Z-z0*id,sigma_z)
    ...
    for ind,val in enumerate(seed_range):
        ...
        # ANDERSON'S DISORDER
        if W!=0:
            disp = sp.diags(np.random.uniform(low=-W/2,high=W/2,size=len(fsyst_sites)))
    ...
    if compute_inv==True and compute_localgap==True:
        print("Invariant:",invariant_average)
        print('Local gap:',np.abs(localgap_average[0]))
